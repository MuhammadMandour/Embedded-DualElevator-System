 ...
 *
 *  FIXES:
 *  1. Adc_ConfigScanGroup_Continuous: CLEAR_BIT(CR2_CONT) changed to SET_BIT(CR2_CONT)
 *  2. ADC_IRQHandler: Adc_AsyncIndex reset to 0 after full scan completes
 */
    ...
    if (READ_BIT(ADC1->SR, SR_EOC))
    {
        if (Adc_CurrentAsyncState == ADC_ASYNC_STATE_SCAN_GROUP)
        {
            if (Adc_AsyncTotal > 0 && Adc_AsyncBuf != 0)
            {
                ...
                if (Adc_AsyncIndex >= Adc_AsyncTotal)
                {
                    /* FIX 2: Reset index so next scan starts from channel 0 */
                    Adc_AsyncIndex = 0;
